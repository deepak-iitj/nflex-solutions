...
for i, line in enumerate(lines):
    if "features: string[" in line:
        lines_to_keep.append("  features: string[];\n")
        in_bad_block = True
    elif in_bad_block:
        if line.strip() == "];":
            in_bad_block = False
...
# Now find the last "];" which ends defaultProducts array and insert new_products
for i in range(len(lines_to_keep)-1, -1, -1):
    if lines_to_keep[i].strip() == "];":
        lines_to_keep[i] = new_products
        break

with open('src/data/products.ts', 'w') as f:
    f.writelines(lines_to_keep)

print("Fixed products.ts")
